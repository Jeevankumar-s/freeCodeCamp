...
console.log(month); // current month (0–11, January = 0)
```
...
console.log(year); // current year (e.g., 2026)
```
...
```js
const date = new Date();
console.log(date.toISOString()); // e.g., 2026-01-18T12:34:56.789Z
```
...
```js
const date = new Date();
console.log(date.toLocaleDateString());  // locale-dependent (e.g., 1/18/2026)
```
...
console.log(date.toLocaleDateString("en-GB", options)); // e.g., Sunday, 18 January 2026
```
